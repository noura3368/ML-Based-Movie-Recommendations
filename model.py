...
def read_mongo(db, collection, query={}, host='localhost', port=27017, username=None, password=None, no_id=True):
    ...
    cursor = db[collection].find({}, {"genres":True, "keywords":True, "title":True, "poster_path":True}).batch_size(5000)
...
def main(movie_name=""):
    ...
    csv_movie_name = [] #movies['title'].tolist()
    csv_movie_poster = []#map(str.lower, movies['poster_path'].tolist())

    for col, row in movies.iterrows():
