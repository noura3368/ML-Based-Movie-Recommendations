...
if movie_name in csv_movie_name:
    ...
    for c in cast:
        if 'character' in c.keys() and c['character'] != '':
            ...
            director += c['name'] + " "
    new_row = pd.DataFrame([{"genres": genres, "keywords": keywords, "tagline": tagline, "title": found_movie['original_title'], "cast": cast_string[:-1], "director": director[:-1]}])
    api_parsed_features = genres + ' ' + keywords + ' ' + tagline + ' ' + director[:-1] + ' ' + cast_string[:-1]
    movies = pd.concat([new_row, movies.loc[:]]).reset_index(drop=True)
    index_of_movie = 0
